...
func_code = """
    fun captureEvidencePackage(targetBlip: RadarBlip? = null): AiEvidencePackage {
        ...
        hardwareCaps.add("Wi-Fi Manager (RTT: ${wifiRttManager.isRttAvailable()})")
...
if "fun captureEvidencePackage" not in content:
    # Insert it before captureRfSnapshot
    content = content.replace("    private fun captureRfSnapshot", func_code + "\n    private fun captureRfSnapshot")
    with open('app/src/main/java/com/example/SignalRadarViewModel.kt', 'w') as f:
        f.write(content)
        print("Updated SignalRadarViewModel.kt")
else:
    print("Already there")
