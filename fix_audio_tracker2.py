...
class AudioRadarTracker(private val context: Context? = null) {
    ...
    private fun startLoop() {
        ...
        trackerJob = CoroutineScope(Dispatchers.IO).launch {
            while (isActive && isPlaying) {
                ...
                if (dist < 0) {
                    currentState = SonarState.UNAVAILABLE
                    delay(1000)
                    continue
                }

                if (dist <= proximityThresholdMeters) {
                    if (currentState != SonarState.TARGET_REACHED) {
                        currentState = SonarState.TARGET_REACHED
    ...
    fun getCurrentState(): SonarState = currentState
}
""")
